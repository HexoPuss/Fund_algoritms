...
Tree* find_first(Tree* node){
    if(node->value.type == symb_VAR){
        return node;
    }
    if(node->value.data.op == '+' || node->value.data.op == '-'){
        Tree* res = find_first(node->left);
        if(res == NULL){
...
bool replace(Tree* tocom){
    


    if(tocom == NULL){return false;}
    Tree* tmp1 = tocom->parent;
    printf("|%d|\n", tmp1 == NULL);
    if((tmp1->value.data.op != '+' && tmp1->value.data.op != '-')){return false;}
    
    Tree* tmp2 = tocom;

    while (tmp1->right == tmp2){
    ...
    Tree* torep = find_first(tmp1->right);
    if(torep == NULL){return false;}
    bool replaced = torep->value.data.c < tocom->value.data.c;
    if(replaced){
        char tmpc = torep->value.data.c;
        torep->value.data.c = tocom->value.data.c;
        tocom->value.data.c = tmpc;
    }
    
    return  replace(torep) || replaced;
}



void sort_task(Tree* node) {
    if (node == NULL) {
        return;
    }
    ...
    if(node->value.type == symb_OP && (node->value.data.op == '+' || node->value.data.op == '-')){
        
        Tree* first = find_first(node);
        while(replace(first)){
            //print_tree(node, 0);
            first = find_first(node);
        }
    }
    
    sort_task(node->left);
    sort_task(node->right);
}
